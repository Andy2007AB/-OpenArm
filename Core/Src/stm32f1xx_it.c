...
void USART1_IRQHandler(void)
{
  /* USER CODE BEGIN USART1_IRQn 0 */
    ...
    if(__HAL_UART_GET_FLAG(&huart1, UART_FLAG_RXNE) != RESET) {
        ...
        HAL_UART_Receive(&huart1, &sbuf_bak, 1, 0);
        ...
        uart1_receive_buf[buf_index++] = sbuf_bak;
...
void USART2_IRQHandler(void)
{
  /* USER CODE BEGIN USART2_IRQn 0 */
    ...
    if(__HAL_UART_GET_FLAG(&huart2, UART_FLAG_RXNE) != RESET) {
        ...
        HAL_UART_Receive(&huart2, &sbuf_bak, 1, 0);
        if(uart2_get_ok)return;
        if(sbuf_bak == '<') {
            uart2_mode = 4;
            buf_index = 0;
        }else if(uart2_mode == 0) {
            if(sbuf_bak == '$') {
                uart2_mode = 1;
            } else if(sbuf_bak == '#') {
                uart2_mode = 2;
        ...
        uart2_receive_buf[buf_index++] = sbuf_bak;
                
        if((uart2_mode == 4) && (sbuf_bak == '>')){
            uart2_receive_buf[buf_index] = '\0';
            uart2_get_ok = 1;
        } else if((uart2_mode == 1) && (sbuf_bak == '!')){
            uart2_receive_buf[buf_index] = '\0';
            uart2_get_ok = 1;
        } else if((uart2_mode == 2) && (sbuf_bak == '!')){
            uart2_receive_buf[buf_index] = '\0';
            uart2_get_ok = 1;
        } else if((uart2_mode == 3) && (sbuf_bak == '}')){
            uart2_receive_buf[buf_index] = '\0';
            uart2_get_ok = 1;
        }
...
void USART3_IRQHandler(void)
{
  /* USER CODE BEGIN USART3_IRQn 0 */
    ...
    if(__HAL_UART_GET_FLAG(&huart3, UART_FLAG_RXNE) != RESET) {
        ...
        HAL_UART_Receive(&huart3, &sbuf_bak, 1, 0);
        if(uart3_get_ok)return;
        if(sbuf_bak == '<') {
            uart3_mode = 4;
            buf_index = 0;
        ...
        uart3_receive_buf[buf_index++] = sbuf_bak;
